"""Completion status for ablation batches."""

import os

RESULTS_DIR = "/n/fs/recbench/new_rec/results"

ABLATION_MODELS = ["HAGNUniform", "HAGNFixed", "CSGCNNoWarmup", "ASLGCNSymmetric"]
...
BATCHES = {
    "ABLATION_BATCH_REAL": (REAL_DATASETS, [None], [1, 12, 123]),
    "ABLATION_BATCH_SYNTH": (SYNTH_DATASETS, [1, 2, 3], [1, 12, 123]),
}
...
def check_batch(batch_name, datasets, data_seeds, model_seeds):
    ...
    total = len(datasets) * len(ABLATION_MODELS) * len(data_seeds) * len(model_seeds)
...
def print_batch_report(batch_name, datasets, data_seeds, model_seeds):
    ...
    if real_only:
        ...
        print(
            f"  Total expected : {total}  ({len(datasets)} datasets × {len(ABLATION_MODELS)} models × {len(model_seeds)} mseeds)"
        )
    else:
        print(f"  Data seeds     : {data_seeds}")
        print(f"  Model seeds    : {model_seeds}")
        print(
            f"  Total expected : {total}  ({len(datasets)} datasets × {len(ABLATION_MODELS)} models × {len(data_seeds)} dseeds × {len(model_seeds)} mseeds)"
        )
    print(f"  Done           : {n_done}")
    ...
    if missing:
        ...
        for ds, ms, dataset, model in missing:
            ...
            print(f"    {prefix}  {model:20s}  {dataset}")
    ...
    for model in ABLATION_MODELS:
        ...
        print(
            f"    {model:20s}  {m_done:4d}/{expected_per_model}  [{bar[:40]}]"
...
def main():
    ...
    if all_missing or all_incomplete:
        out = os.path.join(RESULTS_DIR, "ablation_completion_report.txt")
        with open(out, "w") as f:
            f.write("MISSING RUNS\n" + "=" * 60 + "\n")
            for batch, ds, ms, d, m in all_missing:
                f.write(f"{batch}  dseed={ds}  mseed={ms}  {m:20s}  {d}\n")
            f.write("\nINCOMPLETE RUNS\n" + "=" * 60 + "\n")
