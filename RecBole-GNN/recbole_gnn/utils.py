...
def get_model(model_name):
    ...
    if model_module is None:
        # Fall back to checking each submodule package (for classes exported via __init__)
        for submodule in model_submodule:
            pkg = importlib.import_module('.'.join(['recbole_gnn.model', submodule]))
            if hasattr(pkg, model_name):
